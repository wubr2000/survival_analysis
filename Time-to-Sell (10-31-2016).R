...
#filename = "C:\\Users\\Sujit\\Desktop\\Surival Analysis\\pending_LA_apr15_oct25.csv"
#filename = "C:\\Users\\Sujit\\Desktop\\Surival Analysis\\pending_NY_apr15_oct25.csv"
listingData = read.csv(filename)

index = (1:nrow(listingData))[listingData$ldp_views_day2 > 0]
...
#Calculating decay rate of a fitted exponential model
...
calcValBeta = function(v) {
  return (cov(y=log(v), x=1:7)/var(1:7))
}
...
#Fitting model by AIC in a stepwise algorithm
library(MASS)
stepAIC(coxph(Surv(duration, censor) ~ ldp_views_day1 + photo_count + factor(type) + bedrooms + sqft + price +
                houseAge + monMayJunJul + psf + consConf + beta, temp))
stepAIC(coxph(Surv(duration, censor) ~ factor(type) + photo_count + sqft + ldp_views_day1 + monMayJunJul + psf
                 + beta + consConf, listingData))

summary(coxph(Surv(duration, censor) ~ photo_count + ldp_views_day1 + monMayJunJul
              + priceSqftDiffMedian + beta + consConf, listingData))
...
cph.train = coxph(Surv(durationWks, censor) ~ photo_count + ldp_views_day1 + monMayJunJul
                   + beta + consConf, listingData.train)
summary(cph.train)

#Predicting values percentiles and IQR
calcPredVal = function (rowNum) {
  ...
  survMed = ifelse(length(survMed.curr) == 1, survMed.curr, NA)
  
  surv25th.curr = survTimes[which(survProbs[1:(length(survProbs)-1)] >= 0.75 & cdfProbs >= 0.25)]
  surv25th = ifelse(length(surv25th.curr) == 1, surv25th.curr, NA)
  
  surv75th.curr = survTimes[which(survProbs[1:(length(survProbs)-1)] >= 0.25 & cdfProbs >= 0.75)]
  surv75th = ifelse(length(surv75th.curr) == 1, surv75th.curr, NA)
  
  IQR = surv75th - surv25th
  
  critIndex = match(TRUE, cdfProbs >= 0.80)
  critTime = ifelse(length(critIndex) == 1, survTimes[critIndex], NA)
  critProb = ifelse(length(critIndex) == 1, cdfProbs[critIndex], NA)
  
  return (c(survMed, surv25th, surv75th, IQR, critTime, critProb))
}

testOutput = apply(as.matrix(1:nrow(listingData.test)), 1, calcPredVal)
testOutput = as.data.frame(t(testOutput))
colnames(testOutput) = c("survMed", "Q1", "Q3", "IQR", "critTime", "critProb")

#Finding what percent of predictions were within an accuracyLevel
accuracyLevel = 0.1
biasCorrectionFactor = 0
correctPred = ifelse(listingData.test$duration <= floor((testOutput$critTime + biasCorrectionFactor)*(1+accuracyLevel)), 1, 0)
out = as.data.frame(cbind(listingData.test$duration, listingData.test$censor, testOutput, correctPred))
colnames(out) = c("duration", "censor", colnames(testOutput), "correctPred")
out$critTime = out$critTime + biasCorrectionFactor
out[1:10,]
...
#Accuracy Measures
hotPropThres = 30
hotPredicted = ifelse(out$critTime <= hotPropThres, 1, 0)
